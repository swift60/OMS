...
public class OperatorManager implements Manager {
	...
	public void operator(long x)
	{
		...
		dbcon.getConnection();
		log.debug("database connection made");
		dbcon.setQuery("INSERT INTO DB2ADMIN.OPERATOR (ORDER_NO,SADDRESS_ID,DATE,TIME) VALUES("+rs.getLong("ORDER_NO")+", "+rs.getInt("SADDRESS_ID")+", CURRENT DATE, CURRENT TIME) ");
        dbcon.executeUpdate();	
        deleteoperator(x);
		}
		}
	...
	public ResultSet disporderoperator() {

		 
		 
		dbconn.getConnection();
		log.debug("database connection made");
		dbconn.setQuery("SELECT * FROM DB2ADMIN.OPERATOR AS OPERATOR  ");
		ResultSet rs = dbconn.executeQuery();
		return rs;
	}
